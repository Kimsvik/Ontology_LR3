...
public class RdfWriter {
    ...
    private RdfWriter setResources(StringBuilder xml, Model model) {
        ...
        resourceMap.values().forEach(resource -> {
            if ("ConnectivityNode".equals(resource.cimClassName))
                createXmlResource(resource, xml);
        });
        resourceMap.values().forEach(resource -> {
            if ("Terminal".equals(resource.cimClassName))
                createXmlResource(resource, xml);
        });

        resourceMap.values().forEach(resource -> {
            if ("Breaker".equals(resource.cimClassName))
                createXmlResource(resource, xml);
        });

        resourceMap.values().forEach(resource -> {
            if ("PowerTransformer".equals(resource.cimClassName))
                createXmlResource(resource, xml);
        });
        resourceMap.values().forEach(resource -> {
            if ("OverheadTransmissionLine".equals(resource.cimClassName))
                createXmlResource(resource, xml);
        });
        resourceMap.values().forEach(resource -> {
            if ("RfFilter".equals(resource.cimClassName))
                createXmlResource(resource, xml);
        });
        resourceMap.values().forEach(resource -> {
            if ("WaveTrap".equals(resource.cimClassName))
                createXmlResource(resource, xml);
        });
        resourceMap.values().forEach(resource -> {
            if ("CouplingCapacitor".equals(resource.cimClassName))
                createXmlResource(resource, xml);
        });
        resourceMap.values().forEach(resource -> {
            if ("GroundDisconnector".equals(resource.cimClassName))
                createXmlResource(resource, xml);
        });
        resourceMap.values().forEach(resource -> {
            if ("Disconnector".equals(resource.cimClassName))
                createXmlResource(resource, xml);
        });
        resourceMap.values().forEach(resource -> {
            if ("FourWindingVoltageTransformer".equals(resource.cimClassName))
                createXmlResource(resource, xml);
        });

        resourceMap.values().forEach(resource -> {
            if ("ThreePhaseCurrentTransformer".equals(resource.cimClassName))
                createXmlResource(resource, xml);
        });
        resourceMap.values().forEach(resource -> {
            if ("SurgeArrester".equals(resource.cimClassName))
                createXmlResource(resource, xml);
        });
        resourceMap.values().forEach(resource -> {
            if ("ThreeWindingPowerTransformerWithTapChanger".equals(resource.cimClassName))
                createXmlResource(resource, xml);
        });
        resourceMap.values().forEach(resource -> {
            if ("IndoorCircuitBreaker".equals(resource.cimClassName))
                createXmlResource(resource, xml);
        });
        resourceMap.values().forEach(resource -> {
            if ("ModularSwitchboardWithFuse".equals(resource.cimClassName))
                createXmlResource(resource, xml);
        });
        resourceMap.values().forEach(resource -> {
            if ("SinglePhaseCurrentTransformer".equals(resource.cimClassName))
                createXmlResource(resource, xml);
        });
        resourceMap.values().forEach(resource -> {
            if ("CableTransmissionLine".equals(resource.cimClassName))
                createXmlResource(resource, xml);
        });
        resourceMap.values().forEach(resource -> {
            if ("ModularSwitchboard".equals(resource.cimClassName))
                createXmlResource(resource, xml);
        });
